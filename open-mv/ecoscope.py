from pyb import I2C
from struct import unpack
import pyb
import utime
import sys
import sensor, image, time, math
import os
#import uasyncio

#Color detection spectrum settings

threshold_index = 0 # 0 normal, 1 dim, 2 natural

#Color Tracking Thresholds (L Min, L Max, A Min, A Max, B Min, B Max)
thresholds = [(30, 100, 15, 127, 15, 127), #normal conditions
              (30, 100, -64, -8, -32, 32), #dimmed lighting condition
              (0, 30, 0, 64, -128, 0)] #natural light simulation

sensor.reset()
sensor.set_pixformat(sensor.RGB565)
sensor.set_framesize(sensor.QVGA)
sensor.skip_frames(time = 2000)
sensor.set_auto_gain(False) #auto gain prevents color tracking, so it must be turned off
sensor.set_auto_whitebal(False) #auto whiteball prevents color tracking, so it must be turned off
clock = time.clock()

#If a blob has more pixels in it and a larger area than the thresholds specified, it will be returned
#Define enums:
CAM_MODE_SCANNING = 0x11 #Scanning mode takes pictures and searches for fires
CAM_MODE_DETECTED = 0x22 #Detected mode activates if a fire is detected and the image of the fire gets stored
CAM_MODE_STORED   = 0x33 #Storing mode stores the image and has it ready for download by groundstation
RED     = 0x00
GREEN   = 0x01
BLUE    = 0x02
MOSAIC  = 0x03
#classes:
class FireData: #holds information on fire detection
    mode = CAM_MODE_SCANNING
    photoData = None #tuple of R,G,B, mosiac byte arrays
    subDiv = 0
    row = 0
    col = 0
    def Init(self, img1):
        self.InitPhoto(img1, subDiv = 10)
    def InitPhoto(self,img1,subDiv):
        row_sub = int( img1.height()/subDiv )
        col_sub = int( img1.width() /subDiv )
        arr_r = bytearray((row_sub+1)*(col_sub+1) )
        arr_g = bytearray((row_sub+1)*(col_sub+1) )
        arr_b = bytearray((row_sub+1)*(col_sub+1))
        arr_m = bytearray((row_sub+1)*(col_sub+1))
        print("arr size",(row_sub)*(col_sub))
        print(row_sub)
        print(col_sub)
        self.photoData = (arr_r, arr_g, arr_b, arr_m)
        self.subDiv = subDiv
    def StoreImage(self, img1):
        #stores image in byte array for later transmission
        row_sub = int(img1.height()/self.subDiv)
        col_sub = int(img1.width()/self.subDiv)
        arrInd = col_sub*int(self.row/self.subDiv) + int(self.col/self.subDiv)
        #img_ind = row_sub*int(img1.height()/self.subDiv) + col_sub*int(img1.width()/self.subDiv)
        #store next pixel
        pixel = img1.get_pixel(self.col,self.row)
        #pixel = img1[img_ind]
        #store each channel
        #self.photoData[RED][arrInd]    = pixel[0]
        #self.photoData[GREEN][arrInd]  = pixel[1]
        #self.photoData[BLUE][arrInd]   = pixel[2]
        #store bayer pattern for later interpolation
        if(row_sub%2 == 0 and col_sub %2 == 0):
            self.photoData[MOSAIC][arrInd] = pixel[RED]
        elif(row_sub%2 == 1 and col_sub %2 == 1):
            self.photoData[MOSAIC][arrInd] = pixel[BLUE]
        else:
            self.photoData[MOSAIC][arrInd] = pixel[GREEN]
        #print(self.photoData[MOSAIC][arrInd])
        #print(pixel[0])
        #print(img1[arrInd])
        #index and check for end of image
        self.col = self.col+1*self.subDiv
        if(self.col >= img1.width()):
            self.col = 0
            self.row = self.row +1*self.subDiv
        if(self.row >= img1.height()):
            #finished storing image
            self.row = 0
            self.col = 0
            self.mode = CAM_MODE_STORED
            print ("cam status: " + "IMG STORED" )
            print("image saved, waiting for collection")
            return CAM_MODE_STORED
        return CAM_MODE_DETECTED#keep storing

class I2cHandler():
    #define command enums
    CAM_COM_POLL_ALERT    = 0x11
    CAM_COM_POLL_IMAGE    = 0x13
    CAM_COM_POLL_IMG_INFO = 0x15
    #define alert enums
    #CAM_ALERT_FIRE_DETECT = 0x55
    #define object attributes
    address = 0x26
    dma = False
    i2c = None
    data1 = None
    command = None
    def Init(self):
        #initialize I2C peripheral
        self.i2c = I2C(2)
        self.i2c.init(I2C.SLAVE, addr=self.address, dma= self.dma)
        #switch = pyb.Switch()
        self.data1 = bytearray(6,)
        self.command = bytearray(4)
        utime.sleep_ms(500)
    def HandleI2c(self, fireData):
        #***general flow of handling***:
        #1. check for I2C commands
        #2. if a command is received, determine command
        #3. transmit requested data
        #check for I2C commands
        try: #step 1
            print("I2C: receiving commands..")
            self.data = self.i2c.recv(1, 50)
            self.command[0] = self.data[0] #store in buffer for later procesing
            print("I2C: received")
        except OSError:
            print("I2C: OSE error recv")
            self.data = None
        print (self.data)
        #****AT THIS POINT: breakout to multiple handler functions
        #step 2
        if(self.command[0] == self.CAM_COM_POLL_ALERT):
            print("I2C: fire poll requested")
            self.Transmit(fireData.mode)
        elif(self.command[0] == self.CAM_COM_POLL_IMG_INFO):
            print("I2C: photoInfo requested")
            if(not(fireData.photoData[0] == None)): #send length of photo arrays
                length = len(fireData.photoData[0])
                print(length)
                #serialize length for I2C(uint32 to uint8Arr)
                databuff = bytearray(3)
                databuff[0] = int(length) & 0x0000FF
                databuff[1] = int(length/(2**8)) & 0x0000FF
                databuff[2] = int(length/2**16) & 0x0000FF
                self.Transmit(databuff)
        elif(self.command[0] == self.CAM_COM_POLL_IMAGE):
            #send image data
            print("I2C: image data requested")
            try:
                print("I2C: recv command 2..")
                self.data1 = self.i2c.recv(5, 100) #LSB at [0], numBytes
                print("data 2:")
                print(self.data1[0])
                print(self.data1[1])
                print(self.data1[2])
                print(self.data1[3])
                print(self.data1[4])
            except OSError:
                self.data1 = None
            if(not(self.data1 == None)):
                numBytes = self.data1[0]
                img_buffer = bytearray(numBytes)
                index = self.data1[1] + self.data1[2]*(2**8) + self.data1[3]*(2**16) + self.data1[4]*(2**32)
                print("index: " + str(index))
                #send bayer pattern
                for i in range(numBytes):
                    if(index+i < len(fireData.photoData[MOSAIC])):
                        img_buffer[i] = fireData.photoData[MOSAIC][index+i] #sudo photo data
                    else:
                        img_buffer[i] = 255 #stuff with blank data
                print(img_buffer)
                self.Transmit(img_buffer) #must transmit as fast as possible
            print("I2C: received")
        #reset and wait for next command
        self.command[0] = 0
        utime.sleep_ms(10)
    def CatchRecieveCommand(self):
        #catch empty command from FC receive method
        try:
            print("I2C: catch recieve..")
            self.data = self.i2c.recv(1, 100)
            print("I2C: recieve caught")
        except OSError: #this exception is expected
            print("I2C: catch - no recieve")
            self.data = None
    def Transmit(self, message):
        utime.sleep_ms(10) #need to test delay necessity
        try:
            print("I2C: sending...")
            self.i2c.send(message,0,timeout=5000)
            print("I2C: sent")
        except OSError:
            print("I2C Transmit(): an OSError has occured")
            self.i2c.deinit()
            utime.sleep_ms(100)
            self.i2c.init(I2C.SLAVE, addr=self.address, dma=self.dma)

#***starting code***
i2cEnable  = pyb.Pin("P9", pyb.Pin.IN, pyb.Pin.PULL_DOWN)
firewatch = FireData()
i2cHandler = I2cHandler()
servo = pyb.Servo(1)
img = sensor.snapshot() #needed for init
firewatch.Init(img)
i2cHandler.Init()
enableValue = 0 #current state of I2C request
saveImageOnDetection = True
captureDelay = 10 #num frames of consistent detection needed for capture trigger
captureCounter = captureDelay
#***end start code***

while(True):
    clock.tick()
    #indicate when I2C is toggled
    if(not(i2cEnable.value() == enableValue)):
        print( "I2C enable value: " + str(i2cEnable.value()) )
        enableValue = i2cEnable.value()

    #I2C communication requested
    if( i2cEnable.value() ):
        enableValue = 1
        i2cHandler.HandleI2c(firewatch)
    #camera operation, no I2C request
    elif(firewatch.mode == CAM_MODE_SCANNING):
        enableValue = 0
        img = sensor.snapshot()
        for blob in img.find_blobs([thresholds[threshold_index]], pixels_threshold=200, area_threshold=200, merge=True):
            checker = img.get_histogram()
            stts = checker.get_statistics()
            whte = checker.l_bins()
            #LOGIC FOR CHECKING WHITE IN GRAYSCALE
            if stts [5] <= 100:
                if blob.elongation() > 0.5:
                    #fire detected
                    img.draw_edges(blob.min_corners(), color=(255,0,0))
                    img.draw_line(blob.major_axis_line(), color=(0,255,0))
                    img.draw_line(blob.minor_axis_line(), color=(0,0,255))
                    captureCounter -= 1
                    print(captureCounter)
                    if(saveImageOnDetection and captureCounter <= 0):
                        firewatch.mode = CAM_MODE_DETECTED#stop scanning, start saving
                        print("FIRE DETECTED")
                        print("saving image")
            else:
                #no fire detected
                captureCounter = captureDelay#reset counter
            #These values are stable all the time.
            img.draw_rectangle(blob.rect())
            img.draw_cross(blob.cx(), blob.cy())
            #Note - the blob rotation is unique to 0-180 only.
            img.draw_keypoints([(blob.cx(), blob.cy(), int(math.degrees(blob.rotation())))], size=20)
    elif(firewatch.mode == CAM_MODE_DETECTED):
        print("storing pixel: ", (firewatch.row, firewatch.col))
        firewatch.mode = firewatch.StoreImage(img)
    elif(firewatch.mode == CAM_MODE_STORED):

        flag = 0
        if(flag ==0):
            flag = 1
        pass #wait for image to be collected
    utime.sleep_ms(100)

